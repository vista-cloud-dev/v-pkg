...
// layout are unchanged), built on the shared clikit conventions (--output
// text|json, schema, deterministic errors, the exit-code ladder).
//
// The round-trip guarantee is semantic equality after routine line-2
// canonicalization — NOT byte-identity — exactly as py-kids-vc and XPDK2VC
// define it (volatile patch-list/date/build-number pieces are stripped).
//
// Try:
//
...
func (c *lintCmd) Run(cc *clikit.Context) error {
	...
	if err := cc.Diagnostics(summary, diags, func() {
		cc.Title("kids lint — data-class gate")
		for _, d := range diags {
