...
| **Verification** | `verify` (`--drift`) | `#9.7` status-3 confirmation; **routine drift** vs build source; **content assertion** for every entry type + param-def — the live 0-node is read back and compared ^-piece-aware against the shipped image (`ZeroMatch`, FileMan-transformed pieces masked per type), not just `"B"`-index presence (2026-06-29, **live-proven on vehu** via the package gate) | FILE DD still presence-only (field-def content verify is the next slice); entry verify is 0-node only (higher nodes — menu items, WP — not yet compared) | ◐ content (0-node, live) + routine-drift; FILE presence-only |
| **Back-out** | `uninstall` · `snapshot` · `restore` | **Class-aware** uninstall (routine + entry `DIK` delete; side-effecting **refused** without `--force`); **routine pre-image** capture (`snapshot`) + preview-gated `restore`; pre-image pairing (`--auto-snapshot` ↔ auto-detect); verify-clean | non-routine reversal is **by-design** the authored back-out's job (no generic inverse); **GAP (live-found 2026-06-29): uninstall does NOT deregister the PACKAGE #9.4 footprint** `--register-package` stamps, so `$$PATCH^XPDUTL` reports a ghost after back-out → the *negative* Required-Build direction can't fire (needs a symmetric deregister verb) | ◐ routine-centric; #9.4 deregister missing |
